  ...
  const data = useStaticQuery(graphql`
    query prismicClassesTimetableQuery {
      prismicClassesTimetable {
        ...
        data {
          body {
            ... on PrismicClassesTimetableBodyDayOfTheWeek {
              id
              primary {
                day {
                  text
        ...
        <div className="flex flex-col lg:flex-row lg:flex-wrap">
          <div className="flex flex-col lg:w-3/5">
            <div className="overflow-x-auto">
              {data.prismicClassesTimetable.data.body.map(
                (classesData, index) => (
                  <div
                    key={index}
                    className="w-100 flex flex-col md:flex-row py-2 border-gray-300 border-solid border-b-2 mb-2"
                  >
                    ...
                    <div className="flex flex-col md:w-4/5">
                      {classesData.items.map((itemsData, index) => (
                        <div
                          key={index}
                          className="w-full flex text-sm sm:text-base min-h-42"
                        >
              ...
              (classesKeyData, index) => (
                <div key={index}>
                  <p className="text-orange font-bold">
                    {classesKeyData.class_name.text}
                  </p>
                  <p className="mb-4 text-sm">
                    {classesKeyData.class_description.text}
                  </p>
                </div>
              )
            )}
